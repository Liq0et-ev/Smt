# Assignment progress checklist

- [x] Task 1 — Snowflake Marketplace dataset + Resource Monitors
- [ ] Task 2 — Data Exploration and Enhancement (EDA + Python augmentation)
- [ ] Task 3 — NoSQL (MongoDB) schema design
- [ ] Task 4 — API development (FastAPI)
- [ ] Task 5 — Interactive visualization (Dash/Plotly) + annotations bonus
- [ ] Task 6 — Forecasting + clustering bonus
- [ ] Task 7 — Snowflake performance optimization
- [ ] Task 8 — API caching
- [ ] Task 9 — Pattern recognition (MATCH_RECOGNIZE)
- [ ] Task 10 — GitHub repo + README + deployment docs
- [ ] Final report (PDF/Word) with insights
